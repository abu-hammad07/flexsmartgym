...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $income_sql = "INSERT INTO income (`user_id`, pay_fees, pay_fees_date, remaining_fees, payment_method, trx_image, created_by, created_date)
    VALUES ($income_id, '$pay_fees', '$monthly_date',  '$remaining_fees', '$payment_method', '$screen_shot', '{$_SESSION['username']}', '$created_date')";
    ...
    if (mysqli_query($conn, $income_sql)) {
        ...
        if (mysqli_query($conn, $membership_details)) {
            ...
            if (mysqli_query($conn, $update_users_detail)) {
                ...
                $_SESSION['success_message_income'] = "Income Added Successfully, ($getUsername)";
                header("location: add-income.php");
                exit();
            } else {
                $_SESSION['error_message_income'] = "This ($getUsername) has not been added";
                header("location: add-income.php");
                exit();
            }
        }
            ...
            <!-- Submit Button  -->
                <div class="row mt-3">
                    <div class="col-md-6 col-6 text-start">
                        <a type="button" href="income-details" class="btn btn-danger text-white text-start">Back</a>
                    </div>
...
<script>
    $(document).ready(function() {
        function loadData(type, id) {
            $.ajax({
                ...
                success: function(data) {
                    if (type === "income_name_Data") {
                        $('#income_name').append(data);
                    } else if (type === "income_phone_Data") {
                        $('#income_phone').html(data);
                    } else if (type === "membership_Data") {
        ...
        loadData("income_name_Data");
        loadData("membership_Data");

        $("#income_name").on("change", function() {
            var incomeData = $("#income_name").val();
            if (incomeData != "") {
                loadData("income_phone_Data", incomeData);
            } else {
                $('#income_phone').html("");
            }
        });
        ...
        //         var formattedDate = ('0' + futureDate.getDate()).slice(-2) + '/' + ('0' + (futureDate.getMonth() + 1)).slice(-2) + '/' + futureDate.getFullYear();
        //         $('#membership_end').html("<option value='" + formattedDate + "'>" + formattedDate + "</option>");
        //     } else {
        //         $('#membership_end').html("<option value=''>Please select a membership.</option>");
        //     }
        // });
