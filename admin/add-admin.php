...
// ======================== Add Admin User Details page (add-admin)[save_admin]  ========================
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (mysqli_num_rows($check_username_res) > 0) {
        $_SESSION['error_message_admin'] = "Username already exists, ($username)";
        ...
        if (mysqli_num_rows($check_email_res) > 0) {
            $_SESSION['error_message_admin'] = "Email already exists, ($email)";
            header("location: add-admin");
            exit();
        } else {
            ...
            `full_name`, `phone`, `address`, `gender`, `age`, `city`, `country`, `image`, created_date, created_by) VALUES (
            '$full_name', '$phone_no', '$address', '$gender', '$age', '$city', '$country', '$image', '$created_date', '$created_by')";
            ...
            if ($insert_udetails_res) {
                ...
                $get_role_id = "SELECT `role_id` FROM `role` WHERE `name` = 'Admin'";
                ...
                `username`, `email`, `password`, `token`, `status`, `role_id`, `users_detail_id`, `created_by`, `created_date`) VALUES (
                ...
                if ($insert_user_res) {
                    // redirect("add-admin", "Admin User Successfully Added");
                    $_SESSION['success_message_admin'] = "Admin Addes Successfully, ($username)";
                    header('location: add-admin');
                    exit();
                } else {
                    $_SESSION['error_message_admin'] = "Admin 'users_detail' Not Added, ($username)";
                    header('location: add-admin');
                    exit();
                }
            } else {
                $_SESSION['error_message_admin'] = "Admin 'Users' Not Added, ($username)";
                header('location: add-admin');
                exit();
            }
        }
        ...
        <form method="POST" id="adminForm" enctype="multipart/form-data">
            ...
            <div class="row mt-3">
                ...
                <div class="col-md-6 col-6 text-end">
                    <button type="submit" class="btn btn-primary" id="save_admin" name="save_admin">Save</button>
                </div>
            </div>
        </form>
