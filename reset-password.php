...
if (isset($_POST['reset-password'])) {
  ...
  if (isset($_GET['token'])) {
    $userToken = mysqli_real_escape_string($conn, $_GET['token']);

    // Validate token (assuming token is unique and stored in the database)
    $token_query = "SELECT * FROM users WHERE token = '$userToken'";
    $token_result = mysqli_query($conn, $token_query);

    if ($token_result && mysqli_num_rows($token_result) == 1) {
      // Token is valid, update password
      $hashed_password = password_hash($password, PASSWORD_DEFAULT);
      $update_query = "UPDATE users SET password = '$hashed_password' WHERE token = '$userToken'";
...
<body>
    ...
    <div class="authentication-inner row">
      ...
      <div class="d-flex col-12 col-lg-5 align-items-center p-4 p-sm-5">
        <div class="w-px-400 mx-auto">
          ...
          <?php endif; ?>
          <!-- /Display error messages -->
          <form class="mb-3" action="" method="POST" id="reset-password-form">
            <div class="mb-3 form-password-toggle">
              <label class="form-label" for="password">New Password</label>
              <div class="input-group input-group-merge">
                <input type="password" id="password-reset" class="form-control" name="password" placeholder="&#xb7;&#xb7;&#xb7;&#xb7;&#xb7;&#xb7;&#xb7;&#xb7;&#xb7;&#xb7;&#xb7;&#xb7;" aria-describedby="password" />
                <span class="input-group-text cursor-pointer" id="toggle_password"><i id="eye_icon" class="ti ti-eye-off"></i></span>
              </div>
              <!-- error messages -->
              <span id="pass-reset-error" class="error text-danger"></span>
            </div>
            <div class="mb-3 form-password-toggle">
              ...
              <span id="c-pass-reset-error" class="error text-danger"></span>
            </div>
            <!-- /Display error messages -->
            <button class="btn btn-primary d-grid w-100 mb-3" name="reset-password">Set new password</button>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      const errorPassword = document.getElementById('pass-reset-error');
      const errorConfirmPassword = document.getElementById('c-pass-reset-error');
      ...
      togglePasswordButton.addEventListener('click', function() {
        ...
        togglePasswordButton.querySelector('i').classList.toggle('ti-eye');
        togglePasswordButton.querySelector('i').classList.toggle('ti-eye-off');
      });
  ...
  <script src="assets/js/emptyError.js"></script>
  <script src="assets/js/toes_error.js"></script>
